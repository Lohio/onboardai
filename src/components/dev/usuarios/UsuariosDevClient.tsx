...
import { cargarUsuariosDev } from '@/lib/usuariosDev'
import type { UsuarioRow, EmpresaOption, DatosUsuariosDev } from '@/lib/usuariosDev'
...
interface UsuariosDevClientProps {
  datosIniciales: DatosUsuariosDev
  currentDevId: string
  errorInicial?: boolean
}

export function UsuariosDevClient({ datosIniciales, currentDevId, errorInicial = false }: UsuariosDevClientProps) {
  const { t } = useLanguage()
  const [usuarios, setUsuarios] = useState<UsuarioRow[]>(datosIniciales.usuarios)
  const [empresas, setEmpresas] = useState<EmpresaOption[]>(datosIniciales.empresas)
  ...
  // ─────────────────────────────────────────────
  // Recarga de datos (retry) — la carga inicial llega por props
  // ─────────────────────────────────────────────

  const cargarDatos = useCallback(async () => {
    try {
      const supabase = createClient()
      const datos = await cargarUsuariosDev(supabase)
  ...
  if (error) {
    return (
      <div className="space-y-6">
        ...
        <ErrorState mensaje={t('dev.errorCargarUsuarios')} onRetry={cargarDatos} />
      </div>
    )
  }
      ...
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-sm text-white/40 mt-0.5">
            {`${usuarios.length} ` + t('dev.usuariosCount') + ` · ${usuariosFiltrados.length} ` + t('dev.visibles')}
          </p>
        </div>
        <div className="flex items-center gap-2">
          <ShieldCheck className="w-4 h-4 text-amber-400/60" />
