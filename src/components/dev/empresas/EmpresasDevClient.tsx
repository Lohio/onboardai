...
import { cargarEmpresasDev } from '@/lib/empresasDev'
import type { Empresa, Plan, UsuarioLite, DatosEmpresasDev } from '@/lib/empresasDev'
...
interface EmpresasDevClientProps {
  datosIniciales: DatosEmpresasDev
  errorInicial?: boolean
}

export function EmpresasDevClient({ datosIniciales, errorInicial = false }: EmpresasDevClientProps) {
  const { t } = useLanguage()
  const [empresas, setEmpresas] = useState<Empresa[]>(datosIniciales.empresas)
  const [allUsers, setAllUsers] = useState<UsuarioLite[]>(datosIniciales.allUsers)
  const [error, setError] = useState(errorInicial)
  const [drawer, setDrawer] = useState<{ mode: DrawerMode; empresa?: Empresa } | null>(null)

  // ─────────────────────────────────────────────
  // Recarga de datos (retry / mutaciones) — la carga inicial llega por props
  // ─────────────────────────────────────────────

  const cargarDatos = useCallback(async () => {
    try {
      const supabase = createClient()
      const datos = await cargarEmpresasDev(supabase)
      setEmpresas(datos.empresas)
      setAllUsers(datos.allUsers)
      setError(false)
    } catch (err) {
  ...
  if (error) {
    return (
      <div className="space-y-6">
        ...
        <ErrorState mensaje={t('dev.errorCargarEmpresas')} onRetry={cargarDatos} />
      </div>
    )
  }
      ...
      <div className="flex items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-white/40 mt-0.5">
            {`${empresas.length} ` + (empresas.length !== 1 ? t('dev.empresasRegistradas') : t('dev.empresaRegistrada'))}
          </p>
        </div>
