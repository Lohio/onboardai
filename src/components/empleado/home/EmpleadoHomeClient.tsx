...
import { isModuloDesbloqueado, calcularFaseActual, calcularProgresoPlanGlobal, calcularProgresoPlanFase } from '@/lib/progreso'
...
interface EmpleadoHomeClientProps {
  datosIniciales: DatosHomeEmpleado
...
export function EmpleadoHomeClient({ datosIniciales, errorInicial = false }: EmpleadoHomeClientProps) {
  const { t } = useLanguage()
  ...
  // ── Estado base (inicializado con datos server-side) ──────────
  const [error, setError] = useState(errorInicial)
  const [datosBase, setDatosBase] = useState<DatosBaseEmpleado | null>(datosIniciales.datosBase)
  const [estadoModulos, setEstadoModulos] = useState<EstadoModulos>(datosIniciales.estadoModulos)
  const [progresoPct, setProgresoPct] = useState(datosIniciales.progresoPct)
  const [encuestaPendiente, setEncuestaPendiente] = useState<EncuestaPendiente | null>(null)

  // ── Plan 30-60-90 ────────────────────────────
  const [planItems, setPlanItems] = useState<PlanItem[]>(datosIniciales.planItems)

  // ── Tab activo y dirección de animación ──────
  const tabInicial = useMemo<TabId>(() => {
    if (!datosIniciales.datosBase) return 'M1'
    const e = datosIniciales.estadoModulos
    return !e.M1 ? 'M1' : !e.M2 ? 'M2' : !e.M3 ? 'M3' : !e.M4 ? 'M4' : 'M1'
  }, [datosIniciales])
  const [tabActivo, setTabActivo] = useState<TabId>(tabInicial)
  const [tabPrev, setTabPrev] = useState<TabId>(tabInicial)
  ...
  const [totalBloquesCultura] = useState(datosIniciales.totalBloquesCultura)
  // M3
  ...
  const cargarTabM1 = useCallback(async (userId: string, _usuario: DatosBaseEmpleado) => {
    try {
  ...
  // ── Precargar el primer tab una vez que hay datos base ──────────
  useEffect(() => {
    if (!datosBase) return
    if (tabsCargados.has(tabInicial)) return
    setTabsCargados(new Set([tabInicial]))
    if (tabInicial === 'M1') cargarTabM1(datosBase.id, datosBase)
    else if (tabInicial === 'M2') cargarTabM2(datosBase.id, datosBase.empresa_id)
    else if (tabInicial === 'M3') cargarTabM3(datosBase.id, datosBase.empresa_id)
    else if (tabInicial === 'M4') cargarTabM4(datosBase.id)
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [datosBase, tabInicial])

  // ── Verificar encuesta de pulso al montar (lazy, post-render) ───
  useEffect(() => {
    if (!datosBase) return
  ...
  // ── Retry: recargar datos base client-side ─────────────────────
  const recargarDatosBase = useCallback(async () => {
    setError(false)
    try {
      const supabase = createClient()
      ...
      if (!datos.datosBase) return

      setDatosBase(datos.datosBase)
      setPlanItems(datos.planItems)
      setEstadoModulos(datos.estadoModulos)
      setProgresoPct(datos.progresoPct)

      // Recargar el tab activo tras el retry
      setTabsCargados(new Set())
    } catch (err) {
      console.error('[EmpleadoHome] Error recargando datos base:', err)
      setError(true)
    }
  }, [])
  ...
  if (error) return <ErrorState onRetry={recargarDatosBase} />
