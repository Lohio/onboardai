...
const MENSAJES_AUTO: Record<string, string> = {
  ...
  'Continuar': 'Hola, quiero continuar con el módulo de Cultura. ¿Qué me falta completar?',
  'Hacer una pregunta': '',
}
...
export default function AgenteFlotante({
  ...
  const enviarMensaje = useCallback(async (texto: string) => {
    ...
    setMensajes(prev => [...prev, nuevoUser, nuevoAssistant])
    setInputValue('')
    setEnviando(true)

    try {
      // Construir historial previo para enviar al API
  ...
  const handleCtaPrimario = useCallback((cta: string) => {
    ...
    switch (cta) {
      ...
      case 'Responder ahora':
        ...
        break
      default: {
        // Abrir panel y auto-enviar mensaje contextual si existe
  ...
  const handleCtaSecundario = useCallback((cta: string) => {
    ...
    // '¡Gracias!', 'Entendido' → solo cerrar el hint
  }, [])
