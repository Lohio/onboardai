...
import { cargarDashboardAdmin } from '@/lib/dashboardAdmin'
import type { AlertaConocimientoRow, DatosDashboardAdmin } from '@/lib/dashboardAdmin'
...
interface DashboardClientProps {
  ...
  datosIniciales: DatosDashboardAdmin
}
...
// ─────────────────────────────────────────────
// Dashboard (client) — recibe los datos iniciales del Server Component
// ─────────────────────────────────────────────

export function DashboardClient({ empresaId, datosIniciales }: DashboardClientProps) {
  const router = useRouter()
  const [empleados, setEmpleados] = useState<AdminEmpleadoConProgreso[]>(datosIniciales.empleados)
  const [alertas, setAlertas] = useState<AlertaConocimientoRow[]>(datosIniciales.alertas)
  const [chartData, setChartData] = useState<{ nombre: string; progreso: number }[]>(datosIniciales.chartData)
  ...
  // ─────────────────────────────────────────────
  // Recarga de datos (realtime) — la carga inicial llega por props
  // ─────────────────────────────────────────────

  const cargarDatos = useCallback(async (empId: string) => {
    try {
      const supabase = createClient()
      const datos = await cargarDashboardAdmin(supabase, empId)
      setEmpleados(datos.empleados)
      setAlertas(datos.alertas)
      setChartData(datos.chartData)
    } catch (err) {
