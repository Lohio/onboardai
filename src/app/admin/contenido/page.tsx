...
export default function ContenidoPage() {
  ...
  // ── Bloques del tab empresa activo (solo los sin area/puesto) ──
  const bloquesFiltradosEmpresa = bloques.filter(
    b => b.modulo === tabActivo && !b.area && !b.puesto
  )
  ...
  return (
    ...
    >
      ...
      {/* ── CAPA EMPRESA: tabs M1/M2/M3 (comportamiento original) ── */}
      {capaActiva === 'empresa' && (
        <>
          {/* Tabs de módulos */}
          <motion.div variants={itemVariants}>
            <div className="flex items-center gap-1 p-1 rounded-xl bg-white/[0.03] border border-white/[0.06] w-fit">
              {MODULOS.map(mod => {
                const count  = bloques.filter(b => b.modulo === mod.key && !b.area && !b.puesto).length
                const activo = tabActivo === mod.key
                return (
                  <button
                    key={mod.key}
                    onClick={() => { setTabActivo(mod.key); setFormulario(null) }}
                    className={`relative flex items-center gap-2 px-3 py-2 rounded-lg text-xs font-medium
                      transition-all duration-150 cursor-pointer
                      ${activo
                        ? 'bg-[#0EA5E9]/20 text-[#7DD3FC] border border-[#0EA5E9]/20'
                  ...
                  >
                    ...
                    {count > 0 && (
                      <span className={`min-w-[18px] h-4.5 px-1 rounded-full text-[10px] font-bold
                        flex items-center justify-center
                        ${activo ? 'bg-[#0EA5E9]/30 text-[#7DD3FC]' : 'bg-white/[0.06] text-white/30'}`}>
                        {count}
                      </span>
          ...
          <motion.div variants={itemVariants}>
            <AnimatePresence mode="wait">
              ...
              >
                ...
                <div className="flex items-center justify-between px-5 py-4 border-b border-white/[0.06]">
                  <div className="flex items-center gap-3">
                    <div className={moduloActivo.color}>{moduloActivo.icon}</div>
                    <div>
                      <h2 className="text-sm font-semibold text-white/85">{moduloActivo.label}</h2>
                      <p className="text-xs text-white/35 mt-0.5">{moduloActivo.descripcion}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    ...
                    <Link
                      ...
                      className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs
                        text-white/40 hover:text-white/70 hover:bg-white/[0.04] transition-colors duration-150"
                    >
                      Ordenar
